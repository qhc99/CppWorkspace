...
GLuint BloomRenderer::BloomMip_i(int index)
{
    ...
    return mipChain[(index > size - 1) ? size - 1 : (index < 0) ? 0
                                                                : index]
        .texture;
}
...
int main()
{
    ...
    // load textures
    // -------------
    unsigned int woodTexture = loadTexture((std::filesystem::current_path() / "wood.png").string().c_str(), true); // note that we're loading the texture as an SRGB texture
    unsigned int containerTexture = loadTexture((std::filesystem::current_path() / "container2.png").string().c_str(), true); // note that we're loading the texture as an SRGB texture
