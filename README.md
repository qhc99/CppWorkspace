...
- Enable clangd cuda: update the cuda path in `config.yaml` on your system and copy it to clangd user folder. 
...
1. CMake and Clang support for cuda on windows is limited, so cuda programs will not be generated in this case. 