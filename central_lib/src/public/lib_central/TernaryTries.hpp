...
private:
  enum class Direction { LEFT, MID, RIGHT, NONE };
  ...
  void remove_dangle_node(Node *n, Node *p, Direction direct) {
    if (!n->contain && n->left == nullptr && n->mid == nullptr &&
        n->right == nullptr && p != nullptr) {
      switch (direct) {
      case Direction::NONE:
    ...
    if (p == nullptr && !n->contain && n->left == nullptr &&
        n->mid == nullptr && n->right == nullptr) {
      delete root;
      root = nullptr;
    }
  }
