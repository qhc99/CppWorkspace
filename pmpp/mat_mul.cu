...
__global__ void matMulKernel(float* A, float* B, float* C, size_t i, size_t j, size_t k)
{
    ...
    float* Nds { shared_mem + PMPP_MAT_MUL_KERNEL_TILE_WIDTH * PMPP_MAT_MUL_KERNEL_TILE_WIDTH };
...
void matMul(float* A, float* B, float* C, size_t i, size_t j, size_t k)
{
    ...
    matMulKernel<<<grid_dim, block_dim, 2 * PMPP_MAT_MUL_KERNEL_TILE_WIDTH * PMPP_MAT_MUL_KERNEL_TILE_WIDTH * sizeof(float)>>>(A_d, B_d, C_d, i, j, k);
    checkCudaLastError();
