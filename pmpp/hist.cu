...
__global__ void histKernel(const char* data, unsigned int length, unsigned int* hist_out, unsigned int bin_width)
{
...
void hist(char* data, unsigned int length, unsigned int* hist_out, unsigned int bin_width, bool use_aggregation)
{
    ...
    if (!use_aggregation) {
        histKernel<<<4, 256, bin_count * sizeof(unsigned int)>>>(data_d, length, hist_d, bin_width);
    }
    checkCudaLastError();
